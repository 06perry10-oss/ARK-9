"""ARK-9 RC1 - lightweight desktop chat application."""
...
class ARK9App:
    def __init__(self, root):
        self.root = root
        self.root.title("ARK-9 RC1")
        self.root.geometry("900x600")
        self.root.minsize(700, 450)
        self._build_ui()
        self._add_message("ARK-9", "ARK-9 RC1 is ready.")

    def _build_ui(self):
        header = ttk.Frame(self.root, padding=12)
        header.pack(fill="x")

        ttk.Label(header, text="ARK-9",
                  font=("Segoe UI", 18, "bold")).pack(side="left")
        ttk.Label(header, text="RC1").pack(side="left", padx=8)
    ...
    def _send(self):
        ...
        if text in {"hi", "hello", "hey"}:
            response = "Hello! ARK-9 RC1 is online."
        elif "version" in text:
            response = "You are running ARK-9 RC1."
        elif "help" in text:
            response = "Try hello, version, help, or send any message."
        else:
